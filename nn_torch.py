...
def test(images, labels, model, width):
    ...
    for i in range(width):
        guess = model(torch.tensor(images[i], dtype=torch.float32).view(1, -1)/255) 
...
def main():
    ...
    y2 = []
    y = []
    saved_accuracy = open("best_ff.txt",'r')
    ...
    for k in range(20):
        for i in range(50000):
            ...
            out = digit_classifier(torch.tensor(training_images[i], dtype=torch.float32).view(1, -1)/255)
            ...
            if (i+1) % 100 == 0:
                ...
                if(pct1 > best_accuracy and k > 2):
                    saved_accuracy = open("best_ff.txt",'+w')
                    saved_accuracy.write(str(pct1))
                    best_accuracy = pct1
                    torch.save(digit_classifier.state_dict(), "nn_params.pth")
                    saved_accuracy.close()
